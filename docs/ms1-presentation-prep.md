...
### Why this is needed
The current draw.io C4 Container diagram is the right direction (proper C4 notation, AWS icons, legend), but has seven issues an instructor or peer will catch on first glance. Apply the fixes below in draw.io; target: 10–15 minutes.
...
### Fix 5 — Add the two missing on-prem containers
The C4 diagram omits **Audit / Compliance Store** and **On-Prem Observability**. Both are first-class components in `01-ms1-preliminary-solution.md` §5.3 and are architectural invariants.
...
- Both belong inside the New On-Premises boundary (`n0soMGVSXSvGg1exUm9i-108`, x=1202, y=1560, w=2149, h=880).

### Fix 6 — Add the missing edges
The current C4 has ~10 edges. A complete C4 needs the following additional ones. Source/target IDs are from the .drawio; draw.io will route them.
...
| 9 | `WFdSSb0orDeSZbyKRULI-106` (AI Advisor) | `n0soMGVSXSvGg1exUm9i-20` (Data Stores) | reads (minimized data) | SQL |
| 10 | `n0soMGVSXSvGg1exUm9i-12` (Offline Fraud) | `n0soMGVSXSvGg1exUm9i-20` (Data Stores) | reads | SQL |
| 11 | `n0soMGVSXSvGg1exUm9i-12` (Offline Fraud) | `n0soMGVSXSvGg1exUm9i-11` (Real-Time Fraud) | publishes rules | internal |
| 12 | `n0soMGVSXSvGg1exUm9i-15` (CBS Gateway) | (Audit — new) | writes | append-only |
| 13 | `n0soMGVSXSvGg1exUm9i-10` (Transfer) | (Audit — new) | writes | append-only |
| 14 | `n0soMGVSXSvGg1exUm9i-11` (Real-Time Fraud) | (Audit — new) | writes | append-only |
| 15 | All on-prem Containers | (On-Prem Observability — new) | emits | OTel |
| 16 | (On-Prem Observability — new) | `WFdSSb0orDeSZbyKRULI-99` (Cloud Observability) | ships | mTLS |
| 17 | `WFdSSb0orDeSZbyKRULI-81` (TPPs) | `n0soMGVSXSvGg1exUm9i-14` (Consent) | requests consent | HTTPS |
| 18 | `n0soMGVSXSvGg1exUm9i-18` (Event Bus) | `n0soMGVSXSvGg1exUm9i-12` (Offline Fraud) | feeds | Kafka |

This brings the C4 to a complete, defensible state.
...
| 5 | **C4 Container view** | The main C4 diagram (after the cleanup spec). Walk left-to-right: people, WAF, APIGW, BFF, AI, then on-prem services, then CBS, then data stores. | `docs/diagrams/neobank-digital-leap-c4.drawio`, `docs/diagrams/07-c4-container.mmd` | 3:00 | "Three things to notice: WAF is the only public entry point; the CBS Transaction Gateway is the only path to the mainframe; the AI advisor sits in the cloud private subnet and reads from the ODS, never from CBS. Those three rules protect the bank." |
...
- **Why is the AI advisor in the cloud?** → Deck p.14: "only this service can run on the cloud and not on-prem"; data minimization ensures it never touches CBS (FN-120).
